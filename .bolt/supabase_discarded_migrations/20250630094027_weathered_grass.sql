...
-- Create RLS policies for archive table with IF NOT EXISTS
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'governance_proposals_archive' AND policyname = 'Service role can manage archived proposals'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'governance_proposals_archive' AND policyname = 'Authenticated users can read archived proposals'
  ) THEN
...
-- Create indexes for improved performance - use IF NOT EXISTS
