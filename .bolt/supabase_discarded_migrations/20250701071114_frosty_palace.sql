/*
  # Fix Governance Proposals and Archive Tables

  1. Changes
     - Creates backup tables for governance proposals and votes
     - Ensures RLS is enabled on governance tables
     - Updates RLS policies with proper permissions
     - Fixes orphaned proposals by ensuring proper metadata
     - Creates archive table for old proposals if it doesn't exist
     - Adds proper indexes and policies to archive table
  
  2. Security
     - Enables RLS on all tables
     - Creates appropriate policies for authenticated users and service role
*/
...
-- Create policies for archive table - check if they exist first
DO $$
BEGIN
  -- Check if the policy already exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'governance_proposals_archive' 
    AND policyname = 'Authenticated users can read archived proposals'
  ) THEN
    -- Create the policy if it doesn't exist
    EXECUTE 'CREATE POLICY "Authenticated users can read archived proposals"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'governance_proposals_archive' 
    AND policyname = 'Service role can manage archived proposals'
  ) THEN
    -- Create the policy if it doesn't exist
    EXECUTE 'CREATE POLICY "Service role can manage archived proposals"
      ON governance_proposals_archive
      FOR ALL
      TO service_role
      USING (true)
